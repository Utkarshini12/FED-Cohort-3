...
import '../styles/login.css';
    ...
    // fetch api vs axios api vs rest api 
    // rest api 
    // 4 - get post put and delete : crud opertions => create, read, update and delete => rest api 
    // fetch api vs axios api => applications to perform these operations
    // fetch("url", [get, post, delete, put ]=> options => [header, body, token, method]=> .json => convert json to string )
    // axios.method("url", options => [header, body, token, ])
...
console.log(showSignup);
    ...
    /* 
         grab the data = { username, password} => fetch api => .then(put username, userid and token in localstorage) => take to home => .catch(catch teh error)
    */
    // signup => '/api/v1/user/signup POST 
    /* 
        grab the data = { username, password} => fetch api => .then(take to home) .catch(catch the error)
    */
